...
def get_results_embed(results, title: str = 'Search Results', empty: str = 'No results.'):
    alphabet = [chr(i) for i in range(
        constants.UPPER_A_VALUE, constants.UPPER_Z_VALUE)]
    ...
    message += ''.join(song_for_results(alphabet[results.index(song)], song)
...
async def send_song_embed(message, song, additional=None, uri_images=None):
    embed = get_song_embed(
        song=song, additional=additional, uri_images=uri_images)
